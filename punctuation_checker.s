...
main_punctuation_checker:
	jal tokenizer			# call tokenizer
	jal checker			# call spelling + punctuation checker
...
#### Start of punctuation_checker function ####
checker:
...
loop_start:
        ...
        li $t6, 1                       # 'correct punctuation' boolean
...
isMatch:			
        ...
        li $v0, 10
        seq $t2, $s2, $v0               # dictionary[current_index + 1] == '\n'
        sne $t3, $s0, $zero             # c != '\0'
...
succeeding:
        ...
        sb $zero, tokens($t2)           # add null terminator to checked token
        addi $t0, $t0, 1                # increase row index
        j loop_start	
...
nonAlpha:
        ...
        or $t2, $t2, $t3                # check if c == ',' || c == '.' || c == '!' || c == '?'
        bnez $t2, punctChecker          # branch out to check against punctuation rules

        addi $t0, $t0, 1                # increase row index
        j loop_start                    # brute force check on next token

punctChecker:
        beqz $t0, Rule2                 # Skip rule 1 if punctuation is token no.1

Rule1a:
        mul $t2, $t0, 203               # row * row length
        add $t2, $t2, $t1               # index = (row * row length) + column 
        addi $t2, $t2, -203             # go to previous row

       lb $t3, tokens($t2)              # load first char of preceding token

        li $t7, 32
        seq $t2, $t3, $t7               # check if c == ' '

        beqz $t2, Rule1b                # skip if correct

        andi $t6, $t6, 0                # set boolean to false
        j punctEnd

Rule1b:
        mul $t2, $t0, 203               # row * row length
        add $t2, $t2, $t1               # index = (row * row length) + column 
        addi $t2, $t2, 203              # go to next row

        lb $t3, tokens($t2)             # load first char of succeeding token

        beqz $t3, Rule3a                # Skip rule 2 if last token
...
Rule2: 
        ...
        lb $t5, tokens($t2)             # load first char of succeeding token

        slti $t2, $t5, 91               # check if c <= 'Z'
        li $t7, 65
        sge $t3, $t5, $t7               # check if c >= 'A'
        and $t2, $t2, $t3               # check if c >= 'A' && c <= 'Z'

        slti $t3, $t5, 123              # check if c <= 'z'
        li $t7, 97
        sge $t4, $t5, $t7               # check if c >= 'a'
        and $t3, $t3, $t4               # check if c >= 'a' && c <= 'z'

        or $t2, $t2, $t3                # check if c >= 'A' && c <= 'Z' || c >= 'a' && c <= 'z'
        beqz $t2, Rule3a                # skip if correct
...
Rule3a:
        li $t7, 46
        seq $t2, $s0, $t7               # check if c == '.'
        beqz $t2, Rule4	                # skip if correct / not applicable

        mul $t2, $t0, 203               # row * row length
        add $t2, $t2, $t1               # index = (row * row length) + column 
        addi $t2, $t2, 1                # add offset of one

        lb $t3, tokens($t2)             # load first char of succeeding token
        bnez $t3, Rule3b                # branch if token contains more than one successive full stop

        j Rule4

Rule3b:
        li $t7, 46
        seq $t3, $t3, $t7               # check if next char of token == '.'

        addi $t2, $t2, 1                # go to next char
        lb $t4, tokens($t2)             # load first char of succeeding token

        seq $t5, $t4, $t7               # check if next char is also '.'
        and $t3, $t3, $t5

        addi $t2, $t2, 1                # go to next char
        lb $t4, tokens($t2)             # load first char of succeeding token

        seq $t5, $t4, $zero             # check if next char is '\0'
        and $t3, $t3, $t5

        bnez $t3, Rule4	          # brach if correct
        andi $t6, $t6, 0                # set punctuation boolean to false
        j punctEnd
...
check_end:
        ...
        jr $ra                          # end function call
#### End of punctuation_checker function ####
